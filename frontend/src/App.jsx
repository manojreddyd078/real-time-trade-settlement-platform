...
import SettlementStatusPage from './pages/SettlementStatusPage.jsx'
...
function App() {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span>TS</span><strong>Clearline</strong></div><nav aria-label="Main navigation"><button className={`nav-item ${page === 'overview' ? 'nav-item--active' : ''}`} onClick={() => setPage('overview')}>Overview</button><button className={`nav-item ${page === 'submit' ? 'nav-item--active' : ''}`} onClick={() => setPage('submit')}>Submit trade</button><button className={`nav-item ${page === 'settlements' ? 'nav-item--active' : ''}`} onClick={() => setPage('settlements')}>Settlements</button><button className="nav-item">Exceptions <span className="count">3</span></button><button className="nav-item">Reference data</button></nav><div className="sidebar-footer"><span className={`indicator indicator--${backend.state.toLowerCase()}`} /><div><strong>System {backend.state}</strong><small>{backend.service || 'Connecting…'}</small></div></div></aside><main className="content"><header className="topbar"><div><p className="eyebrow">Settlement operations</p><h1>{page === 'submit' ? 'Trade capture' : page === 'details' ? 'Trade details' : page === 'settlements' ? 'Settlements' : 'Good morning, Alex'}</h1></div><div className="operator"><span>AR</span><div><strong>Alex Rivera</strong><small>Operations analyst</small></div></div></header>{page === 'submit' ? <TradeSubmissionPage onBack={() => setPage('overview')} /> : page === 'details' ? <TradeDetailsPage tradeId={selectedTrade.id} fallback={selectedTrade} onBack={() => setPage('overview')} /> : page === 'settlements' ? <SettlementStatusPage /> : <Overview onCapture={() => setPage('submit')} onSelectTrade={(trade) => { setSelectedTrade(trade); setPage('details') }} />}</main></div>
}
